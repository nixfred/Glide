...
impl LanMouseListener {
    pub(crate) async fn new(
        ...
        let listen_addr = SocketAddr::new(bind_address, port);
                ...
                loop {
                    ...
                    tokio::select! {
                        ...
                        c = listener.accept() => match c {
                            Ok((conn, addr)) => {
                                if allowed_peer_ips.as_ref().is_some_and(|ips| !ips.contains(&addr.ip())) {
                                    log::warn!("Glide: rejected connection outside configured LAN peers: {addr}");
