...
struct ConfigToml {
    ...
    allowed_peer_ips: Option<Vec<IpAddr>>,
    peer_fingerprints: Option<HashMap<IpAddr, String>>,
...
impl Config {
    ...
    /// Glide binds only the selected physical LAN address.
    pub fn bind_address(&self) -> IpAddr {
        self.config_toml
            .as_ref()
            .and_then(|c| c.bind_address)
            .unwrap_or_else(|| "0.0.0.0".parse().unwrap())
    }

    pub fn allowed_peer_ips(&self) -> Option<Vec<IpAddr>> {
        self.config_toml
            .as_ref()
            .and_then(|c| c.allowed_peer_ips.clone())
    }

    pub fn peer_fingerprints(&self) -> HashMap<IpAddr, String> {
        self.config_toml
            .as_ref()
            .and_then(|c| c.peer_fingerprints.clone())
            .unwrap_or_default()
    }
