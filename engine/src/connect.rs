...
async fn connect(
    ...
    let config = Config {
        ...
        // Self-signed device certificates are verified by their SSH-paired pins.
        insecure_skip_verify: true,
        verify_peer_certificate: Some(Arc::new(move |certs, _| {
            if certs.len() == 1
                && crate::crypto::generate_fingerprint(&certs[0]) == expected_fingerprint
            {
                Ok(())
            } else {
...
struct ConnectionIdentity {
    cert: Certificate,
    peers: Rc<HashMap<IpAddr, String>>,
    bind_address: IpAddr,
}
...
async fn connect_any(
    ...
    for &addr in addrs {
        if let Some(expected) = identity.peers.get(&addr.ip()) {
            joinset.spawn_local(connect(
                addr,
                identity.cert.clone(),
                expected.clone(),
                identity.bind_address,
            ));
        } else {
            log::warn!("Glide: no SSH-paired certificate for {addr}; refusing to send input");
        }
    }
    loop {
...
async fn connect_to_handle(
    ...
    if let Some(addrs) = client_manager.get_ips(handle) {
        ...
        let res = connect_any(&addrs, identity).await;
