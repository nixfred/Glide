...
## Omarchy Glide local extension

On Linux, the Glide physical-input observer sends the additive local IPC request
`"TakeLocalOwnership"` when a real mouse or touchpad moves or a physical keyboard key is pressed. If this machine is
receiving remote input, the daemon releases emulated keys and tells its sender
to release capture. Late input is rejected; Enter retries are suppressed for
500 ms after takeover, and repeated Enter packets only resend the acknowledgement.
Incoming return barriers are removed when their handoff ends. Replies from an
inactive outgoing handoff are ignored. The layer-shell backend ignores immediate
re-entry for 250 ms after unlocking to avoid compositor focus loops; move away
from the edge and cross it again if needed.
Motion on the sending machine remains normal and does not interrupt edge sharing.
This does not change the network protocol. The packaged GTK frontend remains
compatible with this daemon.

Glide also provides `bind_address` and `allowed_peer_ips` configuration fields.
Its installer uses a physical LAN IPv4 address and explicitly approved peer
addresses; VPN, container and public interfaces are not discovered or used.

Glide requires `peer_fingerprints` mapping each neighbor IP to its SHA-256
certificate fingerprint obtained over authenticated SSH. Both DTLS directions
are authenticated; absent or mismatched server pins fail closed.
