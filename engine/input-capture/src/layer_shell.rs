...
impl State {
    ...
    fn ungrab(&mut self) {
        if self.pointer_lock.is_none()
            && self.rel_pointer.is_none()
            && self.shortcut_inhibitor.is_none()
        {
            return;
        }
        self.last_ungrab = Some(Instant::now());
    ...
    fn event(
        ...
        match event {
            wl_pointer::Event::Enter {
                ...
                // Focus changes on unlock can immediately re-enter this one-pixel
                // surface without physical motion. Do not lock it again in that
                // cycle; moving away and crossing the edge again rearms capture.
                if !edge_rearmed(app.last_ungrab, Instant::now()) {
                    return;
                }
...
fn edge_rearmed(last_ungrab: Option<Instant>, now: Instant) -> bool {
    last_ungrab.is_none_or(|when| now.duration_since(when) >= Duration::from_millis(250))
}
...
mod glide_tests {
    ...
    #[test]
    fn unlock_focus_cycle_does_not_immediately_recapture() {
        let now = Instant::now();
        assert!(edge_rearmed(None, now));
        assert!(!edge_rearmed(Some(now), now));
        assert!(!edge_rearmed(Some(now), now + Duration::from_millis(249)));
        assert!(edge_rearmed(Some(now), now + Duration::from_millis(250)));
    }
}
